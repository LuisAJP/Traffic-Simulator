...
public class Vehiculo extends ObjetoSimulacion {
	...
	public void avanza() throws ErrorDeSimulacion {
		...
		else if (!estaEnCruce && this.localizacion < carretera.getLength() && !haLlegado) {	// si no esta en cruce y no ha llegado al final de la carretera y no ha llegado al final del itinerario
			actualizarVehiculoRecorrido();
			if (localizacion >= carretera.getLength() && !haLlegado) {//3. Si el coche ha llegado a un cruce 
	...
	public void actualizarVehiculoLlegadoACruce() {
		...
		kilometraje += carretera.getLength() - aux;
	...
	public void actualizarVehiculoEntraNuevaCarretera() {
		...
		this.longitud = this.longitud + carretera.getLength();// contador de las longitudes de las carreteras
	}
